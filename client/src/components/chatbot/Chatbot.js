...
class Chatbot extends Component {                 //syntax to make a class component
    ...
    async df_text_query(queryText) {
        ...
        try {
            const res = await axios.post('/api/df_text_query', { text: queryText, userID: cookies.get('userID') });     //sending queryText and the cookie(unique id ) to backend for unique session
    ...
    componentDidMount() {                     //This method is invoke when chatbot component is mount or render in HTML dom(that means it will invoke first)
        this.df_event_query('Welcome');
    }

    componentDidUpdate() {                   //This will invoke when any thing gets update in the component (This is use for making the scroll at the bottom)
    ...
    renderMessages(stateMessages) {
        ...
        if (stateMessages) {
            return stateMessages.map((message, i) => {
                ...
                if (message.msg && message.msg.text && message.msg.text.text[0] === "result_login") {
                    return (
                        <div style={{ width: "40%", marginLeft: "8%", border: "2px solid" }}>
                            <form onSubmit={this.handleSubmit} style={{ width: "93%", margin: "15px" }}>

                                <div>
                                    <div style={{ paddingLeft: "5px", height: "60px", marginBottom: "5px", backgroundColor: "#ff9800", width: "100%" }}>   <h1>LogIn</h1></div>
                                    <div>
                                        <span><b> <h5>User ID: </h5></b> </span>
                                        <input style={{ border: "1px solid", paddingLeft: "10px", height: "35px", borderRadius: "8px", width: "94%" }} type="text" placeholder="User ID" name="name" required onChange={this.handleChange1} />

                                        <span><b> <h5>Password: </h5></b> </span>
                                        <input style={{ border: "1px solid", paddingLeft: "10px", height: "35px", width: "94%", borderRadius: "8px" }} type="password" placeholder="Password" name="password" required onChange={this.handleChange2} />
                                        <span><b> <h5>Semester: </h5></b> </span>
                                        <input style={{ border: "1px solid", paddingLeft: "10px", height: "35px", borderRadius: "8px", width: "94%" }} type="number" placeholder="Semester" name="semester" onChange={this.handleChange3} />
                                        <button style={{ borderRadius: "8px", backgroundColor: "#ff9800", border: "0px", height: "40px", width: "80px" }} type="submit">Submit</button>
                                    </div>
                ...
                else if (message.msg && message.msg.text && message.msg.text.text) {                      //Checking if text messages are there in response then show text messages 
                        ...
                        <div>
                            <div className="card-panel grey lighten-5 z-depth-1 ">
                                <div style={{ overflow: 'hidden' }}>
                                    ...
                                    <div className="col s2">
                                        <button className="waves-effect waves-light btn">BOT</button>
                                    </div>
    ...
    _handleInputKeyPress(event) {                       //this will invoke when Enter  Key is pressed
        if (event.key === 'Enter') {
